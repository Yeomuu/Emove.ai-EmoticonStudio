...
- Input hand-shape analysis uses MediaPipe Gesture Recognizer across the full capture window. Sustained canned gestures such as `Victory` take priority over coarse raised-arm pose labels, while isolated low-confidence frames must not change the final action.
