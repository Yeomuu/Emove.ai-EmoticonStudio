...
const ALLOWED_KINDS = new Set<GcsAssetKind>(["animations", "characters", "effects", "frames", "thumbnails"]);
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    const asset = await uploadGcsAsset(data, {
      contentType,
    ...
    return json(500, { error: error instanceof Error ? error.message : "GCS 이미지 저장에 실패했습니다." });
  }
}
