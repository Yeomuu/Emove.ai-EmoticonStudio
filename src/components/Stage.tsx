...
export function Stage() {
  ...
  const beginMove = (event: PointerEvent, id: LayerKind) => {
    ...
    const move = (next: PointerEvent) => {
      const width = surfaceRef.current?.getBoundingClientRect().width ?? 360;
      updateLayerTransform(id, { x: start.transform.x + (next.clientX - start.clientX) * 360 / width, y: start.transform.y + (next.clientY - start.clientY) * 360 / width });
    };
  ...
  return (
    <div class="canvas-artboard" ref={surfaceRef} aria-label="직접 편집 가능한 이모티콘 캔버스">
      <canvas class="stage-canvas" ref={canvasRef} width={360} height={360} />
      {[...layers.value].reverse().map((layer, reverseIndex) => {
        if (!layer.visible) return null;
        const rect = layer.id === "text" ? measureTextBubble(motionBrief.value, textBoxShape.value, textFont.value, 360, 360) : selectionRects[layer.id];
        return (
        <div
          key={layer.id}
          ...
          style={{
            ...
            left: `${((rect.x + rect.width / 2 + layerTransforms.value[layer.id].x) / 360) * 100}%`,
            top: `${((rect.y + rect.height / 2 + layerTransforms.value[layer.id].y) / 360) * 100}%`,
            width: `${(rect.width / 360) * 100}%`,
            height: `${(rect.height / 360) * 100}%`,
            transform: `translate(-50%, -50%) rotate(${layerTransforms.value[layer.id].rotation}deg) scale(${layerTransforms.value[layer.id].scale})`,
          }}
