...
export function CharacterPage() {
  ...
  const saveAndContinue = async () => {
    ...
    notify(`새 캐릭터 토큰을 보관함에 저장했어요. ${remoteMessage}`);
    navigate("/library");
  };
        ...
        <div className="character-designer">
          <section className="character-settings-panel glass-panel" aria-labelledby="character-settings-title">
            ...
            <div className="design-setting-group">
              ...
              <div className="palette-control point-color-control"><span>포인트 컬러</span><div className="swatch-row">{selectedPalette.colors.map((color) => <button type="button" className={`color-swatch ${tone.toLowerCase() === color.toLowerCase() ? "active" : ""}`} style={{ background: color }} onClick={() => setTone(color)} aria-label={`${color} 선택`} />)}<label className={`custom-color-swatch ${tone.toLowerCase() === customTone.toLowerCase() ? "active" : ""}`} style={{ background: customTone }} aria-label="직접 색상 선택"><input type="color" value={customTone} onInput={(event) => chooseCustomTone(event.currentTarget.value)} /><Icon name="edit" size={12} /></label></div></div>
            </div>

            <div className="design-setting-group">
              <h2><i />성격 태그</h2>
              <div className="chip-row">{traits.map((item) => <button type="button" className={`chip ${trait === item ? "active" : ""}`} onClick={() => setTrait(item)}>{item}</button>)}</div>
            </div>

            <div className="design-setting-group">
              <h2><i />캐릭터 타입</h2>
              <div className="chip-row">{characterTypes.map((item) => <button type="button" className={`chip ${type === item ? "active" : ""}`} onClick={() => setType(item)}>{item}</button>)}</div>
              <div className="character-select-grid"><label><span>세부 종류</span><select aria-label="세부 종류" value={type === "동물" ? "직접 지정" : type}><option>직접 지정</option><option>{type}</option><option>펭귄</option><option>토끼</option><option>우주인</option></select></label><label><span>스타일</span><select aria-label="스타일" value={style === "3D" ? "Soft 3D" : "Soft 2D"} onChange={(event) => setStyle(event.currentTarget.value.includes("3D") ? "3D" : "2D")}><option>Soft 3D</option><option>Soft 2D</option></select></label></div>
            </div>

            <div className="design-setting-group style-preset-group">
              <h2><i />스타일 프리셋</h2>
              <div className="style-preset-list">{(["2D", "3D"] as const).map((item) => <button type="button" className={style === item ? "active" : ""} onClick={() => setStyle(item)}><span className="preset-cube"><Icon name={item === "3D" ? "layers" : "image"} /></span><b>{item}</b><i /></button>)}</div>
            </div>

            <label className="character-prompt-field"><span>캐릭터 설명</span><input type="text" value={prompt} onInput={(event) => setPrompt(event.currentTarget.value)} placeholder="예: 별을 좋아하는 말랑한 아기 펭귄" /></label>
          ...
          <section className="character-preview-panel glass-panel" aria-label="캐릭터 미리보기">
            <label className="character-name-control"><span>Name</span><div><i style={{ background: tone }} /><input value={name} onInput={(event) => setName(event.currentTarget.value)} maxLength={12} placeholder="새 캐릭터" /><Icon name="edit" /></div></label>
            ...
            <footer className="character-preview-actions"><div><button className="round-tool" type="button" aria-label="실행 취소" disabled={!generated}><Icon name="undo" /></button><button className="round-tool mirror-x" type="button" aria-label="다시 실행" disabled={!generated}><Icon name="undo" /></button></div><button className="save-character-button" type="button" onClick={saveAndContinue} disabled={!generated}>캐릭터 저장하기</button></footer>
          </section>
          ...
          <aside className="character-results-panel glass-panel" aria-labelledby="character-result-title">
            ...
            <div className={`character-variation-grid ${generated ? "" : "is-empty"}`}>{generated ? variationImages.map((image, index) => <button type="button" className={index === selectedVariationIndex ? "active" : ""} onClick={() => selectVariation(index)} aria-label={`${generated.token.name} ${index + 1}번 변형`}><img src={image} alt="" /></button>) : Array.from({ length: 4 }, (_, index) => <button type="button" disabled aria-label={`비어있는 변형 ${index + 1}`}><span className="variation-placeholder"><Icon name="image" /></span></button>)}</div>
            <div className="character-summary"><strong>✦ 캐릭터 요약</strong>{generated ? <p>{characterSummary.map((line) => <><span>{line}</span><br /></>)}</p> : <p>생성 전에는 기존 캐릭터를 보여주지 않습니다.<br />프롬프트와 팔레트 선택 후 새 토큰이 만들어집니다.</p>}</div>
          </aside>
        </div>
    </div>
  );
