...
import { publishGifForQr } from "../services/share";
...
export function EditPage() {
  ...
  const buildAndSave = async (): Promise<EmoticonProject> => {
    ...
    const sharedGif = await publishGifForQr(gifBlob, { fileName: `${safeFileName(title)}.gif`, projectId: id, title });
    const sticker: StickerItem = {
      ...
      image: thumbnail,
      animatedImage: sharedGif.url ?? localGifUrl,
      thumbnail,
      projectId: id,
      gifStoragePath: sharedGif.path ?? originalSticker?.gifStoragePath,
    ...
    if (sharedGif.url || sharedGif.path || sync.downloadUrl || sync.storagePath || sync.ownerId) {
      const remoteGifUrl = sharedGif.url ?? sync.downloadUrl;
      const remoteGifPath = sharedGif.path ?? sync.storagePath;
      const syncedSticker = { ...sticker, ownerId: sync.ownerId ?? sticker.ownerId, animatedImage: remoteGifUrl ?? sticker.animatedImage, gifStoragePath: remoteGifPath ?? sticker.gifStoragePath, updatedAt: new Date().toISOString() };
    ...
    exportGifBlob.value = gifBlob; exportShareUrl.value = sharedGif.url ?? sync.downloadUrl ?? null;
    ...
    notify(remoteError
      ...
      : sharedGif.url
        ...
        : sharedGif.error
          ? `${savedCopy} QR 공유 링크 생성 실패: ${sharedGif.error}`
          : sync.storageWarning
            ? `${savedCopy} ${sync.storageWarning}`
            : original
              ? sync.enabled ? "원본 이모티콘을 현재 위치에 덮어 저장하고 원격 DB도 갱신했어요." : "원본 이모티콘을 현재 위치에 덮어 저장했어요."
              : sync.enabled ? "프로젝트와 1024 GIF 메타데이터를 원격 DB에 저장했어요." : "이모티콘을 기기에 저장했어요."); return project;
  };
  ...
  return (
    <>
      <div className="editor-page">
        ...
        <div className="editor-grid">
          <Panel title="Core effect" meta="VOICE → VISUAL" className="effect-settings">
            ...
            <div className="field-group"><span className="field-label">코어 이펙트 생성 세트</span><div className="preset-list">{effectPresets.map((preset) => <button type="button" className={coreEffect.value === preset ? "active" : ""} onClick={() => chooseCoreEffect(preset)}><i style={{ background: effectColor.value }} />{preset}</button>)}</div></div>
            <label className="color-field"><span><b>이펙트 컬러</b><em>{effectColor.value.toUpperCase()}</em></span><input type="color" value={effectColor.value} onInput={(event) => (effectColor.value = event.currentTarget.value)} /></label>
            <label className="range-field"><span>부가 이펙트 밀도 <strong>{density}</strong></span><input type="range" min="20" max="100" value={density} onInput={(event) => setDensity(Number(event.currentTarget.value))} /></label>
          ...
          <Panel title="Layer properties" meta="CANVAS SYNC" className="layer-properties">
            ...
            {activeLayerId && transform ? <div className="property-grid"><label><span>X</span><input type="number" value={Math.round(transform.x)} onInput={(event) => updateLayerTransform(activeLayerId, { x: Number(event.currentTarget.value) })} /></label><label><span>Y</span><input type="number" value={Math.round(transform.y)} onInput={(event) => updateLayerTransform(activeLayerId, { y: Number(event.currentTarget.value) })} /></label><label><span>크기 %</span><input type="number" min="25" max="240" value={Math.round(transform.scale * 100)} onInput={(event) => updateLayerTransform(activeLayerId, { scale: Number(event.currentTarget.value) / 100 })} /></label><label><span>회전 °</span><input type="number" value={Math.round(transform.rotation)} onInput={(event) => updateLayerTransform(activeLayerId, { rotation: Number(event.currentTarget.value) })} /></label></div> : <p className="no-layer-selected">캔버스 요소나 타임라인 레이어를 선택하면 위치, 크기, 회전을 조정할 수 있어요.</p>}
            {activeLayerId === "text" ? <div className="text-style-controls">
              <label className="text-field"><span>문구</span><textarea rows={3} value={transcript.value} onInput={(event) => (transcript.value = event.currentTarget.value)} /></label>
        ...
        <section className="timeline glass-panel">
          ...
          <div className="timeline-body">
            <div className="layer-stack">
              {displayedLayers.map((layer, index) => <div data-layer-id={layer.id} role="button" tabIndex={0} className={`layer-row ${activeLayer.value === layer.id ? "active" : ""} ${dragId === layer.id ? "is-dragging" : ""} ${dropTarget?.id === layer.id ? `drop-${dropTarget.position}` : ""}`} onClick={() => (activeLayer.value = layer.id)}>
            ...
            <div className="frame-track"><div className="frame-grid">{frameImages.value.map((image, index) => <button type="button" className={selectedFrame.value === index ? "active" : ""} onClick={() => (selectedFrame.value = index)}><img src={image} alt={`${index + 1}번째 동작 프레임`} /><span>FRAME {String(index + 1).padStart(2, "0")}</span></button>)}</div></div>
          </div>
        </section>
      ...
      {exportModalOpen.value && exportGifBlob.value ? <div className="modal-backdrop" onClick={(event) => event.target === event.currentTarget && (exportModalOpen.value = false)}><section className="export-modal glass-panel" role="dialog" aria-modal="true" aria-label="투명 GIF 내보내기"><header><div><span className="eyebrow">EXPORT COMPLETE</span><h2>투명 GIF가 준비됐어요.</h2></div><button className="icon-button" type="button" onClick={() => (exportModalOpen.value = false)}><Icon name="close" /></button></header><div className="export-preview"><img src={URL.createObjectURL(exportGifBlob.value)} alt="완성된 투명 GIF" />{qr ? <div className="qr-card"><img src={qr} alt="GIF 다운로드 QR 코드" /><span>모바일에서 바로 보기</span></div> : null}</div><p>{EXPORT_SIZE}×{EXPORT_SIZE} · {FRAME_COUNT} frames · {frameDelayMs.value}ms/frame · 투명 GIF{exportShareUrl.value ? " · QR 공유 링크" : " · 공유 API 연결 시 QR 생성"}</p><div className="export-actions"><button className="button secondary" type="button" onClick={() => downloadBlob(exportGifBlob.value!, `${safeFileName(emoticonTitle.value || "emove")}.gif`)}><Icon name="download" />기기에 저장</button><button className="button primary" type="button" onClick={share}><Icon name="next" />메일·앱으로 보내기</button></div></section></div> : null}
    </>
  );
}
