...
import { characters, loadProjectIntoEditor, notify, selectCharacter, stickers, toggleFavorite } from "../store";
...
export function LibraryPage() {
  ...
  const findProject = (item: StickerItem) => projects.find((project) => project.id === (item.projectId ?? item.id));
  const editSticker = (item: StickerItem) => {
    const project = findProject(item);
    if (project) loadProjectIntoEditor(project);
    else notify("저장된 편집 프로젝트가 없어 현재 편집 화면으로 이동합니다.");
    navigate("/edit");
  };
  const createGroup = () => {
  ...
  return (
    <>
      {selected ? <LibraryDetail item={selected} project={findProject(selected)} onEdit={() => editSticker(selected)} /> : (
        <div class="workspace-page library-page">
          ...
          <section class="library-content">
            ...
            {mode === "all" ? (
              mixedItems.length ? <div class="sticker-grid library-mixed-grid">{mixedItems.map((entry, index) => entry.kind === "emoticon" ? <StickerCard item={entry.item} index={index} onEdit={editSticker} /> : <CharacterCard item={entry.item} index={index} />)}</div> : <div class="empty-library glass-panel"><Icon name="folder" size={32} /><h2>조건에 맞는 항목이 없어요.</h2><p>검색어나 그룹 조건을 바꿔보세요.</p></div>
            ) : mode === "emoticons" ? (
              visible.length ? <div class="sticker-grid">{visible.map((item, index) => <StickerCard item={item} index={index} onEdit={editSticker} />)}</div> : <div class="empty-library glass-panel"><Icon name="folder" size={32} /><h2>조건에 맞는 움직임이 없어요.</h2><p>다른 감정을 선택하거나 검색어를 바꿔보세요.</p></div>
            ) : (
...
function StickerCard({ item, index, onEdit }: { item: StickerItem; index: number; onEdit: (item: StickerItem) => void }) {
...
function LibraryDetail({ item, project, onEdit }: { item: StickerItem; project?: EmoticonProject; onEdit: () => void }) {
  const stillImage = item.thumbnail ?? item.image;
  ...
  return (
    <div class="workspace-page library-detail-page">
      ...
      <aside class="detail-sidebar glass-panel">
        ...
        <div class="detail-actions"><button type="button" onClick={download} aria-label="저장"><Icon name="download" /></button><button class={item.favorite ? "active" : ""} type="button" onClick={() => toggleFavorite(item.id)} aria-label="즐겨찾기"><Icon name="star" /></button><button type="button" onClick={onEdit}>수정하기</button></div>
